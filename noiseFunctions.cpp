...
// int can be any size >= 16 bits, whereas int8_t, int32_t have a predefined size
uint8_t NoiseFunctions::hash(int32_t i) {
...
float NoiseFunctions::smoothstep(float a, float b, float step) {
    // put into the 0->1 range
    float val = clamp((step - a) / (b - a), 0.0, 1.0);
