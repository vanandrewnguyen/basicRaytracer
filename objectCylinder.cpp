...
bool ObjectCylinder::testIntersection(const Ray& castRay, qbVector<double>& intersectionPoint,
	...
	*/
	...
	// Test intersection
	std::array<qbVector<double>, 4> poi; // 4 element array of type qbVector<double>
	...
	if (isFloatCloseEnough(v.GetElement(2), 0.0)) {
		...
		t.at(3) = (backRay.currPointA.GetElement(2) + 1.0) / -v.GetElement(2);

		// Compute point of intersection
		poi.at(2) = backRay.currPointA + t.at(2) * v;
		poi.at(3) = backRay.currPointA + t.at(3) * v;
	...
	if (minIndex < 2) {
		...
		oldNormal.SetElement(0, validPoi.GetElement(0));
		oldNormal.SetElement(1, validPoi.GetElement(1));
		oldNormal.SetElement(2, 0.0); // normal to circle is flat since it's a 2d shape
